...
| 0.7.0 | 2026-07-14 | 改訂 | 対象仕様を 0.8.0 に更新。カラーテーマ TokyoNight（R10）を設計に追加。EditorPane(4.6) を改訂: 編集ボタンを廃止し読み込み直後から編集可能・保存ボタンのみとする |
...
| SPEC-001-R3 | 読み込み直後から編集可・保存・再描画 | `EditorPane`, `fileSaver`, `markdownRenderer` | R3 正常系（編集可表示／リアルタイム再プレビュー／保存）|
...
| SPEC-001-R10 | カラーテーマを TokyoNight にする | `preview.css`（テーマ変数） | R10 正常系（暗色配色で表示）|
...
- 役割: 本文を編集可能な状態（`<textarea>`）で表示し、編集・リアルタイム再プレビュー・保存を仲介する。
- 背景: ブラウザ拡張のサンドボックスから OS 既定エディタを直接起動する標準APIはないため、**拡張機能内で編集し、保存はダウンロード**する方式を採る（SPEC-001 v0.3.0 で確定）。
- 振る舞い（SPEC-001 v0.8.0 で改訂）:
  - 読み込み直後から原文テキストを `<textarea>` に表示し、**最初から編集可能**とする。編集ボタンは設けない。
  - `<textarea>` の入力（`input`）のたびに `markdownRenderer` に再投入してプレビューを更新（R3 リアルタイム再プレビュー）。
...
### 4.11 カラーテーマ（R10, 優先度 should）
- 役割: 画面全体の配色を TokyoNight（暗い青紫系）にする。
- 方針: 色は `preview.css` の `:root` に CSS カスタムプロパティ（変数）として定義し、各コンポーネントのスタイルはこの変数を参照する。将来テーマを増やす場合の切替を容易にする。
- 主な変数（TokyoNight の代表色）:
  - 背景 `--bg: #1a1b26`、副背景 `--bg-alt: #24283b`、前景 `--fg: #a9b1d6`、
    強調 `--accent: #7aa2f7`、罫線 `--border: #414868`、エラー `--danger: #f7768e`、成功 `--ok: #9ece6a`。
- 適用範囲: 画面全体（`body`）に加え、ドロップ領域・エディタ・ボタン・プレビュー本文（見出し・表・コードブロック・リンク）・フロントマター折りたたみ・通知/エラーバナーまで統一的に暗色配色にする（VSCode 等の暗色エディタと同様の統一感）。
- Mermaid: `diagramRenderer` の初期化で暗色テーマ（`theme: 'dark'`）を用い、図の配色も画面に馴染ませる。
